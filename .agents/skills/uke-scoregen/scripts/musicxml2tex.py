...
# -*- coding: utf-8 -*-
"""
MusicXML → alphaTex 转换器

**这个脚本是 `uke-scoregen` 技能的一部分**（与 `scoregen.py` 同目录）。
把一份 MusicXML 谱面转成 alphaTab 直接可读的 alphaTex 文本。零依赖（只用标准库）。

为什么需要它
------------
alphaTex 是给机器看的东西（`8.1 5.1 3.1` 这种串），人工写、人工核对都费劲；
但它是 alphaTab 的原生格式，也是本技能另一条路（简谱 → 谱面）的最终输出。
而 MusicXML 是外面世界的通用交换格式：MuseScore / Guitar Pro / 各类扒谱工具
导出、以及 `midi-to-musicxml` 那条链路的产物都是它。
所以「MusicXML → alphaTex」就是把外部现成谱面搬进来的那道门。
...
用法
----
    python3 scripts/musicxml2tex.py in.musicxml -o out/score.tex
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--instrument", type=int, default=None,
                    help="写一行 `\\instrument <GM号>`（0 基）。缺省不写，交给消费方自己补")
