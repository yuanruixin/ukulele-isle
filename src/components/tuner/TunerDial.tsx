...
/** 刻度条左右留白：保证指针圆环不会越出容器 */
const PAD = 28;

/** 音分偏差 → 0..1 的横向位置 */
export function centsToRatio(cents: number, range: number): number {
...
export default memo(function TunerDial({
  ...
  const left = `calc(${PAD}px + (100% - ${PAD * 2}px) * ${ratio})`;
  ...
  for (let c = -rangeCents; c <= rangeCents; c += 5) {
    ticks.push({
      left: `calc(${PAD}px + (100% - ${PAD * 2}px) * ${centsToRatio(c, rangeCents)})`,
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex items-start gap-0.5">
        <span
          className="text-[34px] leading-none font-semibold tracking-tight transition-colors"
      ...
      <div className="relative mt-3 h-[64px] w-full">
        ...
        {/* 左右端点：降号 / 升号 */}
        <span
          className="text-secondary absolute top-1/2 left-0 -translate-y-1/2 text-lg leading-none"
          aria-hidden
        >
          ♭
        </span>
        <span
          className="text-secondary absolute top-1/2 right-0 -translate-y-1/2 text-lg leading-none"
          aria-hidden
        >
          ♯
        </span>
        ...
        >
          <span
            className="flex h-[52px] w-[52px] items-center justify-center rounded-full border-[3px] text-[13px] font-semibold tabular-nums transition-colors"
            style={{
