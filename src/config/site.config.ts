...
export const siteConfig = {
  ...
  /**
   * ★ 首页场景里的那几只小猫（场景见 `src/components/scene/HomeScene.tsx`）。
   *
   ...
   *
   *   ⚠️ 这里只管「首页要不要、摆哪几只」—— 一共有三个地方各管一摊，别互相搬：
   *     · **站在哪儿、多大**（不等距、不等高、前后错落）→ `src/styles/globals.css`
   *       的 `.home-scene` 一段，按 `[data-pose]` 选，PC / 窄屏各一套。放那边才好在
   *       浏览器里实时调 —— 这几只猫的站位是要反复改的。
   *     · **动作怎么演**（跳跃高度、踩拍深度、呼吸周期、拖尾量…）→ 引擎顶部的 CAT_CONFIG。
   */
  cats: {
   ...
   * ★★ 首页场景（首页那一幕「猫咖」）★★
   *
   * 立意：不要「一排装饰」，要**一个地方**——猫待在一间猫咖里，各自站在**不同的
   * 高度**上（猫架 / 吧台台面 / 高脚凳 / 木地板），头顶垂着吊灯与挂植，
   * 墙上开着窗、挂着搁板，屋顶吊着两块木牌当菜单。猫本身照旧是描摹稿
   * （几何一字未改），这里换的只是**舞台**。
   *
   * ★ 它是**全屏**的：铺满导航栏以下整屏，不是页面里的一块卡片。
   *   高度算法在 globals.css 的 `.home-stage`（`calc(100dvh - --nav-h)`），
   *   所以这个块里**没有** ratio / ratioMobile —— 场景不再由比例定形状，
   *   横屏 / 竖屏是**两套构图**（见 globals.css 的 `@media (min-aspect-ratio: 5/4)`）。
   *
   * 位置不在这个块里 —— 场景里每样东西的坐标写在 `src/styles/globals.css`
   * 的 `.home-scene` 一段（横屏 / 竖屏两套，用媒体查询切）。
   * 放 CSS 而不是这里，是因为**要改的都是位置**：浏览器里能实时调，也不用等编译。
   * 这个块只管"有什么、去哪、要不要"。
   */
  scene: {
    /** ★ 要不要这块场景（false = 首页退回「只有站名 + 副标题」那一版，一个场景节点都不建） */
    enabled: true,
    /**
     * ★ 吊牌菜单：从场景顶端垂下来的木牌，点一下进对应页面。
     *
     *   为什么是吊牌而不是卡片：屋顶挂 → 天然有垂直空间；摆动 = 现成的「跟随与重叠」；
     *   悬停抬牌 = 明确的可点性；**加一个入口就多挂一块，布局不用动**。
     *
     *   这里只管「有哪些、去哪、什么顺序」。**挂在哪儿、多大、摆多快**在
     *   `src/styles/globals.css` 的 `.scene-sign-<序号>` 一段（序号 = 本数组下标 + 1）——
     *   放那边是因为竖屏得重新排一遍（竖幅里两块牌要重新分布），
     *   写成这个文件里的数字就用媒体查询切不动了。
     */
    signs: [
      { label: "曲谱", to: "/songs" },
      { label: "工具", to: "/tools" },
    ...
    /**
     * ★ 吧台上那台唱片机（**纯装饰，只转，点了没反应**）。
     *   它是「整幕活在同一个节拍里」的那个锚点 —— 转速由 bpm 推出来，不是随便定的。
     */
    turntable: {
      enabled: true,
      /**
       * ★ 唱片转一圈要几秒。
       *
       *   **别随便改这个数**：它跟 `cats.bpm` 是同一个节拍的两个侧面 ——
       *   bpm 96 ⇒ 每分钟 96 拍，取「一圈 = 两拍」⇒ 一圈 = 2×60/96 = 1.25 秒。
       *   于是唱片每转满一圈，猫正好踩完两下拍子；两者同源，
       *   整幕看着才是一个东西在动，而不是几段动画各演各的（这正是"整体感"的来源）。
       *   改 bpm 时这里要跟着改：secondsPerTurn = 120 / bpm。
       */
      secondsPerTurn: 1.25,
    },
  },
